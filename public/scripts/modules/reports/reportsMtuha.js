                ...
                $scope.showBookFive = function () {
                    ...
                    $mdDialog.show({
                        controller: function ($scope, $rootScope) {
							...
							$scope.start_date = ((new Date()).getFullYear())+'-'+(((new Date()).getMonth()+1).toString().length == 2 ? ((new Date()).getMonth()+1).toString(): '0'+((new Date()).getMonth()+1).toString())+'-01';
							
							$scope.end_date = ((new Date()).getFullYear())+'-'+(((new Date()).getMonth()+1).toString().length == 2 ? ((new Date()).getMonth()+1): '0'+((new Date()).getMonth()))+'-'+(new Date(((new Date()).getFullYear()), ((new Date()).getMonth()+1), 0)).getDate();
							$scope.getReportBasedOnthisDate=function (dt_start,dt_end) {
                                ...
                                var Report = {attempt:0, load: function(){
									Report.attempt++;$http.post('/api/getMahudhurioOPD',reportsOPD).then(function(data) {
										$scope.opd_mahudhurio_new = data.data[0][0];
										$scope.opd_mahudhurio_marudio = data.data[1][0];
										if(data.data[0].length > 0){
											$scope.opd_mahudhurio = $.extend($scope.opd_mahudhurio_new);
                                ...
                                }, function(data){if(Report.attempt < 5) Report.load();});
								}}
				...
				$scope.showBookFourteen = function () {
                    ...
                    $mdDialog.show({
                        controller: function ($scope, $rootScope) {
							...
							$scope.startup = function(){
									$scope.getIPDReportBasedOnthisDate(undefined, undefined);
								}
